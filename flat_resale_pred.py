...
if select == 'HOME':
    ...
    with col2:
        ...
        storey_start = st.number_input ('Storey_start')

        storey_end = st.number_input('Storey_end')
    ...
    if st.button(':violet[Predict]',use_container_width=True):
            ...
            prediction = re_sale_price([[month,town,flat_type,block,floor_area_sqm,flat_model,lease_commence_date,remaining_lease,
                                         year,age_of_property,storey_start,storey_end,holding_period,rate_per_sqm]])
